<form action="{{ @$customer ? url('customers/'.$customer->id) : url('customers') }}" method="post">
    ...
    <div class="modal-header bg-dark text-light">
        <h5 class="modal-title"><i class="fa fa-plus"></i> {{ @$customer ? 'Edit' : 'Tambah' }} Customer</h5>
    ...
    <div class="modal-body">
        <div class="form-group row">
            <div class="col-lg-12">
                ...
                <input id="project_owner" class="form-control" name="project_owner"
                    value="{{ @$customer->project_owner ?? old('project_owner') }}">
            </div>
        </div>
        <div class="form-group row">
            <div class="col-lg-12">
                <label for="no_ktp" class="form-control-label">No. KTP</label>
                <input id="no_ktp" class="form-control" name="no_ktp" value="{{ @$customer->no_ktp ?? old('no_ktp') }}">
        ...
        <div class="form-group row">
            <div class="col-lg-12">
                ...
                <input id="npwp" class="form-control" name="npwp" value="{{ @$customer->npwp ?? old('npwp') }}">
                <small class="text-secondary">Boleh dikosongkan</small>
            </div>
        </div>
        <div class="form-group row">
            <div class="col-lg-12">
                ...
                <input id="phone" class="form-control" name="phone" value="{{ @$customer->phone ?? old('phone') }}">
            </div>
        </div>
        <div class="form-group row">
            <div class="col-lg-12">
                ...
                <textarea name="address" id="address" class="form-control"
                    rows="5">{{ @$customer->address ?? old('address') }}</textarea>
            </div>
        </div>
        <div class="form-group row">
            <div class="col-lg-12">
                <label for="fax" class="form-control-label">Fax Customer</label>
                <input id="fax" class="form-control" name="fax" value="{{ @$customer->fax ?? old('fax') }}">
                <small class="text-secondary">Boleh dikosongkan</small>
            </div>
        </div>
        <div class="form-group row">
